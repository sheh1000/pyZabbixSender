...
from pyZabbixSenderBase import pyZabbixSenderBase
...
class SenderProtocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        while len(data):
            ...
            if l:
                ...
                self.error_happens(failure.Failure(Exception("Unknown data chunk")))
    ...
    def _expected_parse_magic(self,data):
        if not data == 'ZBXD\1':
            self.error_happens(failure.Failure(Exception("Wrong magic: %s" % data)))
            self.transport.loseConnection()
            return
        self.state = 'header'
...
class SenderProcessor(SenderProtocol):
    ...
    def packet_received(self,packet):
        ...
        if failed is None or processed is None:
            raise Exception('Unable to parse server response',packet)
